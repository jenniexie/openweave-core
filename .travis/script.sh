...
case "${BUILD_TARGET}" in
    ...
    linux-auto-gcc-check)
        echo "./configure --enable-coverage && make && make check"
        ;;
    ...
    linux-lwip-clang)
        echo "./configure --with-target-network=lwip --with-lwip=internal --disable-java && make"
        ;;
    ...
    linux-lwip-gcc-check)
        ...
        # may be created.
        echo "./configure --enable-coverage --with-target-network=lwip --with-lwip=internal --disable-java && make && sudo make check"
        ;;
    ...
    linux-auto-*-lint)
        echo "./configure && make pretty-check"
        ;;
